...
void inserir_chave_secundaria(struct aluno aluno, struct chave_secundaria *chaves_s, int *count_s1, struct lista_invertida *lista_s, int *count_s2) { //Insere chave secundária em vetores na memória
    int posicaoNome;
...
int verifica_flag() { //Verifica se o programa foi interrompido na execução anterior
    FILE *fd;
    int flag, arquivoExiste, nova_flag = 0;
...
void ler_indice_p(struct chave_primaria *chaves_p, int *count) { //Carrega as chaves primárias para um vetor na memória
    FILE *fd;
...
void carrega_indice_p(struct chave_primaria *chaves_p, int count) { // Carrega as chaves primárias para o arquivo de índice
    FILE *fd;
...
void recria_indice_s(struct chave_secundaria *chaves_s, int *count1, struct lista_invertida *lista_s, int *count2) { // Lê o arquivo de dados e recria as chaves secundárias já que os arquivos de índice estão desatualizados
    ...
    while (fread(&tamRegistro, sizeof(int), 1, fd) == 1) {
        ...
        inserir_chave_secundaria(aluno, chaves_s, count1, lista_s, count2);
    }

    fclose(fd);
    carrega_indice_s(chaves_s, *count1, lista_s, *count2);
    *count1 = 0;
    *count2 = 0;
}

void recria_indice_p(struct chave_primaria *chaves_p, int *count) { // Lê o arquivo de dados e recria as chaves primárias já que o arquivo de índice está desatualizado
    FILE *fd;
    ...
    fclose(fd);
    carrega_indice_p(chaves_p, *count);
    *count = 0;
}
...
void busca_secundaria(char *nome, struct chave_secundaria *chaves_s, int count1, struct lista_invertida *lista_s, struct chave_primaria *chaves_p, int count2) { // Busca um aluno pelo nome
    FILE *fd;
    ...
    for (int i = 0; i < count1; i++) {
        if (strcmp(chaves_s[i].nome_aluno, nome) == 0) {
            ...
            while (prox != -1) {
                ...
                strcpy(pesquisa.id_aluno, lista_s[prox].id_aluno);
                strcpy(pesquisa.sigla_disc, lista_s[prox].sigla_disc);
                busca_primaria(&pesquisa, chaves_p, count2);
...
int main() {
    ...
    int count_insere, id, i = 0, j = 0, k = 0, l = 0;
    ...
    if (flag) {
        recria_indice_p(chaves_p_atuais, &count_p_atuais);
        recria_indice_s(chaves_s_atuais, &count_s_atuais, lista_s, &count_lista);
    }

    ler_indice_p(chaves_p_atuais, &count_p_atuais);
        ...
        switch (entrada) {
            case 'i':
                ...
                if (i < (count_insere - 1)) {
                    ...
                    inserir_chave_secundaria(alunos[id], chaves_s_atuais, &count_s_atuais, lista_s, &count_lista);
                    i++;
            ...
            case 's':
                ...
                if (l < (count_s_busca - 1)) {
                    busca_secundaria(chaves_s_busca[id].nome_aluno, chaves_s_atuais, count_s_atuais, lista_s, chaves_p_atuais, count_p_atuais);
                    l++;
                }
                else {
                    printf("Todos os alunos já foram buscados\n");
            ...
            case 'e':
                entrada = 'e';
                carrega_indice_p(chaves_p_atuais, count_p_atuais);
